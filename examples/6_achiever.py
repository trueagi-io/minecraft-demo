...
class AttackBlockSight(RobGoal):
    ...
    def finished(self):
        ...
        return self.target != cur_t and cur_dist > self.max_dist
...
class GridAnalyzer:
    ...
    def inWater(self):
        return self.grid3D[self.dimY//2][self.dimZ//2][self.dimX//2] == 'water'

    def underWater(self):
        return self.grid3D[self.dimY//2+1][self.dimZ//2][self.dimX//2] == 'water'
    ...
    def analyzePath(self, pos):
        ...
        if rd < self.dist_thresh or (self.dist < self.dist_thresh and dy < 2.66 and dy > -0.99):
            return ['done', 0]
        if self.target[1] - self.pa[1] > 1.5+self.dist_thresh and self.dist < 1.5:
            return ['fly', 0]
