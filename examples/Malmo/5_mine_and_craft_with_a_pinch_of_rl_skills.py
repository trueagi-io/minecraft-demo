...
def mineAtSight(mc):
    ...
    for t in range(100):
        if mc.getLineOfSight('type') is None or abs(
                dist - mc.getLineOfSight('distance')) > 0.01 or obj != mc.getLineOfSight('type'):
...
def getstate(mc, target):
    return None


def runScared(mc, target):
    path = '../experiments/agent_cliff_v2.pth'
    agent = load_agent(path)
    optimizer = torch.optim.Adam(agent.parameters())
    trainer = Trainer(agent, mc, optimizer, 0., False)
    trainer.target_x = target[1]
    trainer.target_y = target[3]
    trainer.write_visualization = False
    prev_pos = None
    while True:

        try:
            data = trainer.collect_state()
            new_pos = data['pos']
        ...
        new_actions = trainer.agent(data, epsilon=0.)
        trainer.act(new_actions)
        if mc.isLineOfSightAvailable() and mc.getLineOfSight('distance') < 0.5 and \
                not mc.getLineOfSight('distance') in passableBlocks and \
                not mc.getLineOfSight('type') in passableBlocks:
            break
