...
class JumpUpOrObtain(RobGoal):
    ...
    def __init__(self, agent):
        ...
        for block in build_blocks:
            item = rob.mlogy.findInInventory(invent, {'type': block})
            if item:
                super().__init__(rob,
...
class SelectMineTool(RobGoal):
    ...
    def update(self):
        ...
        if los is not None and 'type' in los and los['type'] != self.last_target and inv is not None:
            ...
            tool = self.rob.mlogy.select_minetool(inv, mine_entry)
...
class Obtain(Switcher):
    ...
    def update(self):
        ...
        if self.delegate is None:
            ...
            for item in new_items:
                ...
                for mine_entry in self.rob.mlogy.find_mines_by_result(item):
                    blocks3.extend(self.blocksToStrings(mine_entry[0]['blocks']))
                for mine_entry in self.rob.mlogy.find_mines_by_result(item):
                    ...
                    if blocks[0] == 'diamond_ore' or blocks[0] == 'deepslate_diamond_ore':
                        ...
                        if sticks_in_inv is None or sticks_in_inv['quantity'] < 5:
                            diff = -1
        ...
        if new_items == [] or (self.depthlimit is not None and self.rec_depth >= self.depthlimit):
            return False
        return super().update()
