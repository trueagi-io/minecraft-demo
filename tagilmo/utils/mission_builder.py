...
class ColourMapProducer:
    def __init__(self, height, width, respectOpacity=True):
        self.height = height
        self.width = width
        self.respectOpacity = respectOpacity

    def xml(self):
        opacity = 'false'
        if self.respectOpacity:
            opacity = 'true'

        return '<ColourMapProducer respectOpacity="{opacity}"> \
                   <Width>{width}</Width> \
                   <Height>{height}</Height> \
                </ColourMapProducer>'.format(opacity=opacity, width=self.width, height=self.height)
...
class DepthProducer:
    ...
    def xml(self):
        return '<DepthProducer> \
                   <Width>{width}</Width> \
                   <Height>{height}</Height> \
                </DepthProducer>'.format(width=self.width, height=self.height)
...
class AgentHandlers:
    ...
    def __init__(self, commands=Commands(), observations=Observations(),
                 all_str='', video_producer=None, colourmap_producer=None, depth_producer=None,
        ...
        self.depth_producer = depth_producer
    ...
    def xml(self):
        ...
        _xml += '' if self.depth_producer is None else self.depth_producer.xml()
        _xml += '</AgentHandlers>\n'
    ...
    def hasDepth(self):
        if self.depth_producer is None:
            return False
        return True
...
class AgentSection:
    ...
    def hasDepth(self):
        if self.agenthandlers.hasDepth():
            return True
        return False
